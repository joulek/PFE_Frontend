...
function ScoreCircle({ percentage }) {
  ...
  const color =
    percentage >= 80 ? "#4E8F2F"
    : percentage >= 65 ? "#69B332"
    : percentage >= 50 ? "#9CCB73"
    : "#C96A3D";

  return (
    <div className="relative w-36 h-36 flex items-center justify-center">
      <svg className="w-full h-full -rotate-90">
        <circle cx="72" cy="72" r={radius} fill="none" stroke="#e5e7eb" className="dark:stroke-slate-700" strokeWidth="12" />
        <circle cx="72" cy="72" r={radius} fill="none" stroke={color} strokeWidth="12"
          strokeDasharray={circ} strokeDashoffset={offset} strokeLinecap="round" />
      </svg>
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className="text-4xl font-extrabold" style={{ color }}>{percentage}%</span>
        <span className="text-xs font-medium mt-1" style={{ color }}>SCORE</span>
      </div>
    </div>
  );
}
...
function resolveAnswer(val, order, questions = []) {
  ...
  if (/^[A-E]$/.test(str)) {
    ...
    if (typeof opt === "object") return opt.text || opt.label || opt.value || str;
  }
...
export default function QuizResultPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        // Candidature info — séparée pour ne pas bloquer si quiz 403
        try {
          const cRes = await api.get(`/candidatures/${candidatureId}`);
          setCandidature(cRes.data);
        } catch (err) {
          console.warn("Candidature fetch error:", err?.response?.status);
        }

        // Quiz submissions — peut être 403 si DGA pas autorisé
        try {
          const qRes = await api.get(`/quiz-submissions/candidature/${candidatureId}`);
          ...
          if (err?.response?.status === 403) {
            console.warn("Quiz 403 — DGA non autorisé sur cette route. Ajouter 'DGA' dans requireRoles côté backend.");
            setQuiz("FORBIDDEN"); // sentinel value
          } else {
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {quiz === "FORBIDDEN" ? (
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-[#dfead6] dark:border-slate-700 p-10 text-center">
            ...
            <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">Accès restreint</h3>
            <p className="text-gray-500 dark:text-gray-400 text-sm max-w-md mx-auto">
              Les résultats du quiz ne sont pas accessibles avec votre rôle actuel.
              <br />
              <span className="text-xs text-gray-400 mt-1 block font-mono">→ Ajouter "DGA" dans requireRoles de /quiz-submissions/candidature/:id</span>
            </p>
          </div>
        ) : !quiz ? (
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-[#dfead6] dark:border-slate-700 p-10 text-center">
            ...
            <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">Aucun quiz soumis</h3>
            <p className="text-gray-500 dark:text-gray-400">Ce candidat n'a pas encore soumis de réponses de quiz.</p>
          </div>
        ) : (
          ...
          <div className="bg-white dark:bg-slate-800 border border-[#dfead6] dark:border-slate-700 rounded-2xl shadow-sm overflow-hidden">
            ...
            <div className="p-6">
              ...
              <div className="flex flex-col md:flex-row gap-8 md:gap-12 mb-8">
                <div className="shrink-0"><ScoreCircle percentage={quiz.percentage} /></div>
                <div className="flex-1 grid grid-cols-1 sm:grid-cols-3 gap-5">
                  <div className="text-center p-4 bg-[#EEF7E9] dark:bg-green-900/20 rounded-xl border border-[#d6e8ca] dark:border-green-800/40">
                    <p className="text-3xl font-bold text-[#4E8F2F] dark:text-green-300">{quiz.score}</p>
                    <p className="text-sm text-[#4E8F2F] dark:text-green-400 mt-1">CORRECTES</p>
                  </div>
                  <div className="text-center p-4 bg-[#FFF7ED] dark:bg-orange-900/20 rounded-xl border border-[#F5D6B3] dark:border-orange-800/40">
                    <p className="text-3xl font-bold text-[#C55B00] dark:text-orange-300">{quiz.totalQuestions - quiz.score}</p>
                    <p className="text-sm text-[#C55B00] dark:text-orange-400 mt-1">INCORRECTES</p>
                  </div>
                  <div className="text-center p-4 bg-[#F7F8F6] dark:bg-slate-800 rounded-xl border border-[#dfe5d7] dark:border-slate-700">
                    <p className="text-3xl font-bold text-[#0b1430] dark:text-gray-200">{quiz.totalQuestions}</p>
                ...
                {quiz.answers?.map((a, i) => (
                  ...
                  >
                    <div className="flex items-start gap-4">
                      <div className={`w-9 h-9 rounded-lg flex items-center justify-center text-white font-bold shrink-0 ${a.isCorrect ? "bg-[#6CB33F]" : "bg-[#D97706]"}`}>
                        {a.order || i + 1}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="font-medium text-gray-900 dark:text-gray-100 mb-2.5 leading-snug">{a.question}</p>
                        <div className="flex flex-wrap gap-3 mt-2">
                          <div className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-sm font-medium ${
                            ...
                            {resolveAnswer(a.selectedAnswer, a.order, quizQuestions)}
                          </div>
                          {!a.isCorrect && a.correctAnswer && (
                            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-sm font-medium bg-[#DFF0D4] dark:bg-green-800/30 text-[#2F6B1B] dark:text-green-200">
                              ...
                              {resolveAnswer(a.correctAnswer, a.order, quizQuestions)}
                            </div>
                          )}
                        </div>
                      </div>
