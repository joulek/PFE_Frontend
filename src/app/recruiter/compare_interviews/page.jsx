...
function CompareContent() {
  ...
  useEffect(() => {
    ...
    // ── Pas d'IDs → rediriger vers la liste des comparaisons ──
    if (ids.length < 2) {
      router.replace("/recruiter/comparisons_list");
      return;
    }

    // ── Lancer la comparaison et rediriger vers le détail ──
    async function run() {
      try {
        ...
        // ✅ Rediriger vers la page de détail avec l'ID sauvegardé
  ...
  // ── Loading (pendant l'analyse IA) ──
  return (
    <div className="min-h-screen bg-[#F0FAF0] flex items-center justify-center">
      <div className="flex flex-col items-center gap-4 text-center">
        <div className="w-16 h-16 rounded-full bg-[#E9F5E3] flex items-center justify-center">
          <Brain className="w-8 h-8 text-[#6CB33F] animate-pulse" />
        </div>
        <div>
          <p className="text-lg font-extrabold text-gray-900">Analyse IA en cours…</p>
          <p className="text-sm text-gray-500 mt-1">Le LLM compare les candidats, veuillez patienter</p>
        </div>
