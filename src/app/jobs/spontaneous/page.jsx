...
import { Send, ArrowLeft, Upload, X, CheckCircle2 } from "lucide-react";
import { createSpontaneousApplication } from "../../services/application.api";
...
export default function SpontaneousApplicationPage() {
  ...
  async function handleSubmit(e) {
    ...
    formData.append("type", "SPONTANEE");
    if (cvFile) formData.append("cv", cvFile);

    try {
      setLoading(true);
      await createSpontaneousApplication(formData);
      setSubmitted(true);
    } catch (err) {
  ...
  const inputBase =
    ...
    "focus:ring-4 focus:ring-[#6CB33F]/15 outline-none transition-colors";
    ...
    return (
      <div className="min-h-screen bg-[#F0FAF0] dark:bg-gray-950 flex items-center justify-center px-4">
        <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-xl p-12 max-w-md w-full text-center">
          <div className="h-20 w-20 rounded-full bg-[#E9F5E3] dark:bg-emerald-950/40 grid place-items-center mx-auto mb-6">
            <CheckCircle2 size={40} className="text-[#6CB33F]" />
          </div>
          <h2 className="text-2xl font-extrabold text-gray-900 dark:text-white mb-3">
            Candidature envoyée !
          </h2>
          <p className="text-gray-500 dark:text-gray-400 text-sm mb-8">
            Merci pour votre intérêt. Notre équipe RH examinera votre candidature et vous contactera prochainement.
          </p>
          <button
            onClick={() => router.push("/jobs")}
            className="w-full h-12 rounded-full bg-[#6CB33F] hover:bg-[#4E8F2F] text-white font-semibold transition-colors"
          >
            Retour aux offres
          </button>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-lg p-8">
          ...
          <div className="flex items-center gap-4 mb-8">
            <div className="h-12 w-12 rounded-2xl bg-[#E9F5E3] dark:bg-emerald-950/40 grid place-items-center">
              <Send size={20} className="text-[#6CB33F]" />
            </div>
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                Envoyez votre candidature, nous vous contacterons.
              </p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-5">
            ...
            <div>
              ...
              <input
                ...
                placeholder="Ex: Développeur Frontend, Data Analyst..."
                className={inputBase}
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Parlez-nous de vous, votre parcours, vos motivations..."
            ...
            <div>
              ...
              {cvFile ? (
                ...
                <label className="flex flex-col items-center justify-center h-24 rounded-2xl border-2 border-dashed border-gray-300 dark:border-gray-600 cursor-pointer hover:border-[#6CB33F] dark:hover:border-emerald-500 transition-colors bg-white dark:bg-gray-700/50">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full h-12 rounded-full bg-[#6CB33F] hover:bg-[#4E8F2F] dark:bg-emerald-600 dark:hover:bg-emerald-500 text-white font-semibold transition-colors shadow flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed"
            >
              {loading ? (
                <>
