...
useEffect(() => {
  ...
  (async () => {
    setLoading(true);
    try {
      // 1. Récupérer le vrai candidatureId depuis l'interview
      let realId = candidatureId;
      try {
        const ivRes = await api.get(`/api/interviews/candidature/${candidatureId}`);
        const ivList = ivRes.data?.data || [];
        if (ivList[0]?.candidatureId) {
          realId = String(ivList[0].candidatureId);
        }
      } catch (_) {}

      // 2. Charger candidature + fiche avec le vrai ID
      const [cRes, fRes] = await Promise.all([
        api.get(`/candidatures/${realId}`).catch(() => ({ data: null })),
        api.get(`/fiche-submissions/candidature/${realId}`).catch(() => ({ data: [] })),
      ]);
