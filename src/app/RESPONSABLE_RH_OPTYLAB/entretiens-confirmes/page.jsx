...
export default function ConfirmedInterviewsPage() {
  ...
  function goToDetails(iv) {
    const id = iv.candidatureIdStr || String(iv.candidatureId);
    router.push(`/RESPONSABLE_RH_OPTYLAB/entretiens-confirmes/${id}`);
  }
                ...
                {filteredInterviews.map((iv, idx) => (
                  <div key={String(iv._id)} className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4 shadow-sm">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="pt-2">
                        <p className="text-[11px] uppercase tracking-wider font-bold text-gray-400 mb-2">Actions</p>
                        <ActionButtons onView={() => goToDetails(iv)} />
                      </div>
                    </div>
                  </div>
