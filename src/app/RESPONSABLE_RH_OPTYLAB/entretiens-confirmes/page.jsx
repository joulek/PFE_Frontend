...
function TypeBadge({ type }) {
  ...
  return (
    <span
      className={`inline-flex items-center justify-center rounded-full border px-3 py-1 text-[12px] font-semibold whitespace-nowrap ${isRhTech
...
function FilterChip({ active, label, count, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`inline-flex items-center gap-2 rounded-full border px-4 py-2 text-xs font-semibold transition-colors ${active
...
export default function ConfirmedInterviewsPage() {
  ...
  function goToDetails(iv) {
    if (!iv?.candidatureId) return;
    router.push(`/RESPONSABLE_RH_OPTYLAB/entretiens-confirmes/${iv.candidatureId}`);
  }
  ...
  const counts = useMemo(() => {
    ...
    const rhTech = interviews.filter(
      (i) => i.interviewType === "rh_technique"
    ).length;
    const rh = interviews.filter(
      (i) => i.interviewType !== "rh_technique"
    ).length;
                  ...
                  <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                    {filteredInterviews.map((iv) => (
                      ...
                      >
                        ...
                        <td className="px-6 lg:px-8 py-5">
                          <ActionButtons onView={() => goToDetails(iv)} />
                        </td>
                      </tr>
                    ))}
                  </tbody>
